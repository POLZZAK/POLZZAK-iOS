...
final class UserInfoManager {
    ...
    enum PolzzakToken {
        enum TokenType {
            ...
            var keychainKey: String {
                switch self {
                case .access:
                    return Constants.KeychainKey.polzzakAccessToken
                case .refresh:
                    return Constants.KeychainKey.polzzakRefreshToken
                }
            }
        }
    ...
    enum FCMToken {
        static let keychainKey = Constants.KeychainKey.fcmToken
        
        static func saveToken(_ token: String) {
            Keychain().create(identifier: keychainKey, value: token)
        }
        
        static func readToken() -> String? {
            return Keychain().read(identifier: keychainKey)
        }
        
        static func deleteToken() {
            Keychain().delete(identifier: keychainKey)
        }
    }
    ...
    static func checkFirstLaunch() {
        ...
        if isFirstLaunch {
            PolzzakToken.deleteToken(type: .access)
            PolzzakToken.deleteToken(type: .refresh)
